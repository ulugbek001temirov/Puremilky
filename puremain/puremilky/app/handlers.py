...
def modify_brands_for_q7(brands: list[str]) -> list[str]:
    """Remove 'Essi', rename 'AgroBravo' to 'Простоквашино', and add 'Пропустить' for Q7."""
    brands = brands.copy()  # Don't modify original
    if "Essi" in brands:
        brands.remove("Essi")
    brands = ["Простоквашино" if b == "AgroBravo" else b for b in brands]
    if "Пропустить" not in brands:
...
def add_ne_videl_to_brands(brands: list[str]) -> list[str]:
    """Add 'не видел' option to brands list for Q8."""
    brands = brands.copy()  # Don't modify original
    if "не видел" not in brands:
        brands.append("не видел")
    return brands
...
async def q3_multi(c: CallbackQuery, state: FSMContext):
    ...
    if c.data == "milk_done":
        ...
        await state.update_data(q3=", ".join(s))
        brands = load_brands(region)
        await c.message.edit_text("4. " + texts[lang]["q2"], reply_markup=kb_idx(brands, "q2"))
        await state.set_state(Survey.q2)
        return
...
async def q3_other_multi(m: Message, state: FSMContext):
    ...
    await state.update_data(q3=", ".join(s), q3_selected=list(s))
    region_brands = load_brands(user_region)
    await m.answer("4. " + texts[lang]["q2"], reply_markup=kb_idx(region_brands, "q2"))
    await state.set_state(Survey.q2)
...
async def q4(c: CallbackQuery, state: FSMContext):
    ...
    # Use modified brands for Q7
    brands = modify_brands_for_q7(load_brands(region))
...
async def q5(c: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(q5=val)
    # For Q8, use ORIGINAL brands (with Essi and AgroBravo) + "не видел"
    brands_for_q8 = add_ne_videl_to_brands(load_brands(region))
    await c.message.edit_text("8. " + texts[lang]["q6"], reply_markup=kb_idx(brands_for_q8, "q6"))
    await state.set_state(Survey.q6)
...
async def get_q6_other(m: Message, state: FSMContext):
    ...
    # For Q8, use ORIGINAL brands (with Essi and AgroBravo) + "не видел"
    brands_for_q8 = add_ne_videl_to_brands(load_brands(region))
    await m.answer("8. " + texts[lang]["q6"], reply_markup=kb_idx(brands_for_q8, "q6"))
    await state.set_state(Survey.q6)
...
async def q6(c: CallbackQuery, state: FSMContext):
    ...
    if val == "Другое":
        ...
        # User chose "не видел" - finish survey
        await state.update_data(q6=val)
        success = add_respondent(c.from_user.id, await state.get_data())
        if not success:
            await c.message.answer(texts[lang]["error"])
        ...
        print("Survey result:", data_result)
        await c.message.edit_text(texts[lang]["finish"])
        await state.clear()
    else:
        # User chose a brand - continue to Q9
        await state.update_data(q6=val)
        await c.message.edit_text("9. " + texts[lang]["q7"], reply_markup=kb_idx(COMMERCIAL, "q7"))
        await state.set_state(Survey.q7)
